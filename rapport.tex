...
state is a continuous 3.3 V signal going through the paddle.

\subsection{Reading the signal states}
Once the game is launched, the signal amplitude is situated between 0.655 V
and 3.300 V. The following images represent the way the game console
understands the 0 and 1 states of a single bit. A bit is read on a period of
4 \( \mu \)s.

0 state: the voltage value of the signal is lowered to 0.655 V for 3 \( \mu \)s and
then released to its initial state.
\\\includegraphics[width=\textwidth]{0bit.png}
\\1 state: the voltage value of the signal is lowered to 0.655 V for 1 \( \mu \)s and
then released to its initial state.
...
N.B. The voltage is lowered to 0 V during the request sequence, not 0.655 V as
for the inputs signal.

\subsection{Inputs signal}
...
\begin{itemize}
    ...
    \begin{itemize}
	    ...
	    \item bit 1: B button
	    \item bit 2: Z button
	    \item bit 3: Start button
	    \item bit 4: D pad up button
    	\item bit 5: D pad down buton
    	\item bit 6: D pad left button
    	\item bit 7: D pad right button
    \end{itemize}
    \item Byte 1:
    \begin{itemize}
        \item bit 0: Nothing
	    ...
	    \item bit 2: L button
	    \item bit 3: R button
	    \item bit 4: C up button
    	\item bit 5: C down buton
    	\item bit 6: C left button
    	\item bit 7: C right button
    \end{itemize}
